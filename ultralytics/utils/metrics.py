...
def bbox_iou(
        ...
        DIoU: bool = False,
        CIoU: bool = False,
        WIoU: bool = False,  # <--- 1. 这里新增了参数
        eps: float = 1e-7,
) -> torch.Tensor:
    """
    Calculate IoU and other metrics (WIoU, CIoU, DIoU, GIoU).
    """
    # Get the coordinates of bounding boxes
...
class OBBMetrics(DetMetrics):
    ...
    def __init__(self, names: dict[int, str] = {}) -> None:
        ...
        self.task = "obb"
def bbox_nwd(box1, box2, C=12.5, eps=1e-7):
    """
    计算 Normalized Wasserstein Distance (NWD)
    box1, box2: shape [N, 4] (xyxy)
    C: 常数，通常取 12.5 (与数据集目标平均尺寸有关，微小目标可适当调小如 10.0)
    """
    # 1. 将 xyxy 转换为 xywh (中心点 + 宽高)
    b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
    b1_w = b1_x2 - b1_x1
    b1_h = b1_y2 - b1_y1
    b1_cx = b1_x1 + b1_w / 2
    ...
    # 2. 计算 Wasserstein 距离 (L2 范数)
    # 对于高斯分布: W2^2 = ||m1-m2||^2 + ||Sigma1^0.5 - Sigma2^0.5||_F^2
    # 简化版 (用于水平框):
    w2 = (b1_cx - b2_cx).pow(2) + (b1_cy - b2_cy).pow(2) + \
         ((b1_w - b2_w).pow(2) + (b1_h - b2_h).pow(2)) / 4

    # 3. 归一化 (NWD)
    # nwd = exp( - sqrt(W2) / C )
    nwd = torch.exp(-torch.sqrt(w2 + eps) / C)
